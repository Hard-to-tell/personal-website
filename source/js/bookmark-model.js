...
(function (root, factory) {
  ...
  function toNetscapeHtml(input) {
    const nodes = validateTree(input);
    const lines = [
      "<!DOCTYPE NETSCAPE-Bookmark-file-1>",
      "<!-- Generated by 在流放地. Import this file into Chrome, Edge or Firefox. -->",
    ...
    const render = (parentId, depth) => {
      ...
      for (const node of nodes.filter((item) => item.parentId === parentId)) {
        if (node.type === "folder") {
          lines.push(`${indent}<DT><H3>${escapeHtml(node.title)}</H3>`, `${indent}<DL><p>`);
          render(node.id, depth + 1);
          lines.push(`${indent}</DL><p>`);
        } else {
          lines.push(`${indent}<DT><A HREF="${escapeHtml(node.url)}">${escapeHtml(node.title)}</A>`);
          if (node.note) lines.push(`${indent}<DD>${escapeHtml(node.note)}`);
        }
      }
    };
    render(null, 1);
  ...
  return { normalizeUrl, validateTree, applyOperation, fromLegacy, toNetscapeHtml };
});
